...
function sanitizeStatuteVersionsPayload(payload: unknown): StatuteVersionPayload[] {
  ...
  return payload.map((entry) => {
    const typedEntry =
      entry && typeof entry === "object"
        ? (entry as Partial<StatuteVersionPayload>)
        : {};
...
export async function GET(request: Request) {
  try {
    ...
    const url = new URL(`${BACKEND_BASE}/statute/versions`);
    ...
    return NextResponse.json(sanitizeStatuteVersionsPayload(payload), {
