...
type PrivacyPolicyMutationBody = {
  content?: unknown;
  summary?: unknown;
};
...
function sanitizePrivacyPolicyPayload(payload: unknown): PrivacyPolicyPayload {
  if (!payload || typeof payload !== "object") {
    return { id: "", content: "", updatedAt: "" };
  }

  const typedPayload = payload as Partial<PrivacyPolicyPayload>;
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${BACKEND_BASE}/privacy-policy`, {
      method: "GET",
    ...
    return NextResponse.json(sanitizePrivacyPolicyPayload(payload), {
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as PrivacyPolicyMutationBody;
    const session = await getServerSession(authOptions);
